...
// ---------- DATABASE SETUP ----------
// DB_PATH lets you point this at a persistent disk on Render (or anywhere else)
// instead of the default location, which lives on the app's local, non-persistent
// filesystem. See README.md -> "Why did my account disappear?" for details.
const dbPath = process.env.DB_PATH || path.join(__dirname, 'peerfinder.db');
...
app.post('/api/auth/signup', (req, res) => {
  ...
  // One email = one profile. The pre-check above catches the common case;
  // this catch handles the rare race where two signups with the same email
  // land at the same instant (the email column also has a UNIQUE constraint
  // at the database level as the final backstop).
  let result;
  try {
    result = db.prepare(`
...
app.put('/api/me', requireAuth, (req, res) => {
  ...
  // Subjects apply from semester 2 onward — ignore them otherwise, even if sent.
...
// ---------- MESSAGING (basic peer-to-peer chat) ----------

// Fetch the conversation between the signed-in user and another user.
app.get('/api/messages/:otherUserId', requireAuth, (req, res) => {
  ...
  if (!otherUser) return res.status(404).json({ error: 'That user no longer exists.' });
...
app.post('/api/messages', requireAuth, (req, res) => {
  ...
  if (!recipientIdNum) return res.status(400).json({ error: 'A recipient is required.' });
  if (recipientIdNum === req.userId) return res.status(400).json({ error: "You can't message yourself." });
  if (!body || !body.trim()) return res.status(400).json({ error: 'Message cannot be empty.' });

  const trimmedBody = body.trim().slice(0, 2000);

  const recipient = db.prepare('SELECT id FROM users WHERE id = ?').get(recipientIdNum);
  if (!recipient) return res.status(404).json({ error: 'That user no longer exists.' });

  const result = db.prepare(`
    INSERT INTO messages (sender_id, recipient_id, body) VALUES (?, ?, ?)
  `).run(req.userId, recipientIdNum, trimmedBody);

  const row = db.prepare('SELECT id, sender_id, recipient_id, body, created_at FROM messages WHERE id = ?').get(result.lastInsertRowid);
  res.json({
    id: row.id,
